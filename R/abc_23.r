...
calc_abc2 <- function(
  ...
  n.catch=5   #  period for averaging the past catches
    ...
    n <- length(catch)   # the number(length) of catch data after omitting na
    l.cpue <- length(cpue)   # the number(length) of cpue data after omitting na
...
calc_abc3 <- function(
  ...
  PL=6,   #  BL = PL*BT
  PB=10,   #  BB = PB*BT
  tune.par = c(2,1), #  tuning parameters: (gamma1, gamma2)
#  BT=0.05,   # initial target level
#  PL=8,   #  BL = PL*BT
#  PB=20,   #  BB = PB*BT
#  tune.par = c(3.5,3.5), #  tuning parameters:  (gamma1, gamma2)
  n.catch=5   #  period for averaging the past catches
){
    argname <- ls() # 引数をとっておいて再現できるようにする
    arglist <- lapply(argname,function(xx) eval(parse(text=xx)))
...
abc_t23_proto1 <- function(
  ...
  gamma1 <- tune.par[1]   # velocity to go to BT
  gamma2 <- tune.par[2]   # correction factor when D <= BL
  gamma3 <- tune.par[3]   # tuning parameter for updating BT
